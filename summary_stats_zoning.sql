...
\COPY (select city, count(*) as countof, sum(landacres*max_dua) as units from (select city, hs, ht, hm, mr, max_dua, st_area(geom)/4046.86 as landacres from zoning.pgc_residential where max_dua != -9999) a group by city ORDER BY units DESC) TO 'residential_units_by_city_by_type_from_dua' WITH CSV;

\COPY (select city, count(*) as countof, sum(max_du_per_parcel) as units from (select city, hs, ht, hm, mr, max_du_per_parcel, st_area(geom)/4046.86 as landacres from zoning.pgc_residential where max_du_per_parcel != -9999) a group by city ORDER BY units DESC) TO 'residential_units_by_city_by_type_from_du_per_parcel' WITH CSV;

\COPY (select city, count(*), sum(landarea*max_far) as commercial_sqft from (select city, max_far, st_area(geom) as landarea from zoning.pgc_commercial where max_far != -9999) a group by city order by commercial_sqft DESC) TO 'commercial_square_footage_by_city_landarea_x_far' WITH CSV;

\COPY (select city, count(*), sum(landarea*(max_height/10)) as commercial_sqft from (select city, max_height, st_area(geom)*0.7 as landarea from zoning.pgc_commercial where max_height != -9999) a group by city order by commercial_sqft DESC) TO 'commercial_square_footage_by_city_height_by_10_x_area' WITH CSV;
...
\COPY (select distinct(hs,ht,hm,mr),city,count(*),sum(landacres*max_dua) as units from (select city, hs, ht, hm, mr, max_dua,st_area(geom)/4046.86 as landacres from zoning.parcel_generic_code) a group by hs,ht,hm,mr,city) TO 'residential_units_by_city_by_type' WITH CSV;
--1 acre = 4046.86
